...
def make_playlist(song_string, playlist_name, song_folder):
    ...
    os.makedirs("final_project/playlists", exist_ok=True)
    with open(f"final_project/playlists/{playlist_name}.txt", "w") as file:
        for item in playlist:
            print(f"{item}: {playlist[item]}", file=file)


def main():
    os.makedirs("final_project/songs", exist_ok=True)
    os.makedirs("final_project/playlists", exist_ok=True)
    playing = False
    player = None
    while True:
        ...
        if playing == True:
            ...
            if select == "1":
                ...
                try:
                    ...
                    print(song)
                    player = vlc.MediaPlayer(f"final_project/songs/{song}")
                    player.play()
            ...
            if player_mode == "1":
                ...
                os.system("clear")
                song_folder = list_folder("final_project/songs")
